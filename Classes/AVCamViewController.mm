...
@synthesize lkWinSize = _lkWinSize;
@synthesize lkLevel = _lkLevel;
@synthesize lkMaxPoints = _lkMaxPoints;
...
@synthesize lkWinSizeParam;
@synthesize lkLevelParam;
@synthesize lkMaxPointsParam;
...
- (void)viewDidLoad {
    ...
    touchController = [[AVCamTouchController alloc] initWithFrame:[[self videoPreviewView] bounds]];
    [[self videoPreviewView] addSubview:touchController];
	
	// Start app with initial points
	[self resetPoints:nil];
...
- (void)lkWinSizeChange:(id)sender
{
	switch ([(ExpandyButton *)sender selectedItem]) {
        case 0:
			self.lkWinSizeParam = 2;
            break;
		case 1:
			self.lkWinSizeParam = 3;
            break;
		case 2:
			self.lkWinSizeParam = 4;
            break;
		case 3:
			self.lkWinSizeParam = 5;
            break;
		case 4:
			self.lkWinSizeParam = 6;
			break;
		case 5:
			self.lkWinSizeParam = 8;
			break;            
		case 6:
			self.lkWinSizeParam = 12;
			break;                        
...
- (void)lkMaxPointsChange:(id)sender
{
	switch ([(ExpandyButton *)sender selectedItem]) {
        case 0:
			self.lkMaxPointsParam = 10;
            break;
		case 1:
			self.lkMaxPointsParam = 20;
            break;
		case 2:
			self.lkMaxPointsParam = 50;
            break;
		case 3:
			self.lkMaxPointsParam = 100;
            break;
		case 4:
			self.lkMaxPointsParam = 200;
            break;
    }
...
- (void)updateExpandyButtonVisibility
{
    if ([self isHudHidden]) {
		...
		[[self lkWinSize]			setHidden:YES];
		[[self lkLevel]				setHidden:YES];
		[[self lkMaxPoints]			setHidden:YES];
		...
		// Win Size button
		expandyButton = [self lkWinSize];
		if (expandyButton == nil) {
			ExpandyButton *lkWinSize =  [[ExpandyButton alloc] initWithPoint:CGPointMake(8.f, 8.f + (40.f * count))
																			  title:@"WinSize"
																		buttonNames:[NSArray arrayWithObjects:@"2", @"3", @"4", @"5", @"6",@"8",@"12", nil]
																	   selectedItem:6];
			[lkWinSize addTarget:self action:@selector(lkWinSizeChange:) forControlEvents:UIControlEventValueChanged];
			[view addSubview:lkWinSize];
			[self setLkWinSize:lkWinSize];
			[lkWinSize release];
		} else {
			CGRect frame = [expandyButton frame];
			[expandyButton setFrame:CGRectMake(8.f, 8.f + (40.f * count), frame.size.width, frame.size.height)];
		...
		// Max points button
		/*
		expandyButton = [self lkMaxPoints];
		if (expandyButton == nil) {
			ExpandyButton *lkMaxPoints =  [[ExpandyButton alloc] initWithPoint:CGPointMake(8.f, 8.f + (40.f * count))
																	 title:@"Max Points"
															   buttonNames:[NSArray arrayWithObjects:@"10", @"20", @"50", @"100", @"200", nil]
															  selectedItem:2];
			[lkMaxPoints addTarget:self action:@selector(lkMaxPointsChange:) forControlEvents:UIControlEventValueChanged];
			[view addSubview:lkMaxPoints];
			[self setLkMaxPoints:lkMaxPoints];
			[lkMaxPoints release];
		} else {
			CGRect frame = [expandyButton frame];
			[expandyButton setFrame:CGRectMake(8.f, 8.f + (40.f * count), frame.size.width, frame.size.height)];
		...
		*/
		...
		if (expandyButton == nil) {
			ExpandyButton *displayMethod =  [[ExpandyButton alloc] initWithPoint:CGPointMake(8.f, 8.f + (40.f * count))
																		title:@"Display"
																  buttonNames:[NSArray arrayWithObjects:@"Points", @"Trails", nil]
																 selectedItem:0
																  buttonWidth:70];
